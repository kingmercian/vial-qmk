...
def automagic_keyboard(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        if cli.config_source[cli.subcommand_name.__name__]['keyboard'] != 'argument':
            keyboard = find_keyboard_from_dir()

            if keyboard:
                cli.config[cli.subcommand_name.__name__]['keyboard'] = keyboard
                cli.config_source[cli.subcommand_name.__name__]['keyboard'] = 'keyboard_directory'
...
def automagic_keymap(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        if cli.config_source[cli.subcommand_name.__name__]['keymap'] != 'argument':
            keymap_name, keymap_type = find_keymap_from_dir()

            if keymap_name:
                cli.config[cli.subcommand_name.__name__]['keymap'] = keymap_name
                cli.config_source[cli.subcommand_name.__name__]['keymap'] = keymap_type
